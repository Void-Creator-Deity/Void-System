...
def load_persona_chain():
    ...
    def ensure_session(input_data, config=None):
        if input_data["config"] is None:
            if input_data["config"]["configurable"] is None: 
                if input_data["config"]["configurable"]["session_id"] is None:
                     config["configurable"]["session_id"]=input_data["config"]["configurable"]["session_id"]
        if config is None:
            config = {"configurable": {}}
    ...
    def shuchu(x):
        print(f"[Persona链] 实际收到的输入: {x}, 类型: {type(x)}")
        return ensure_session(x)
    # ✅ 返回一个真正的 Runnable 对象
    return RunnableLambda(shuchu)
