...
def load_qa_chain():
    ...
    def extract_question_only(input_data):
        return input_data  # 返回字符串

    chain = (
        RunnableLambda(extract_question_only)  # 输出字符串
        .assign(
            context=lambda x: retriever.invoke(x["question"]),  # x 是字符串
            question=lambda x: x["question"]                    # 传递字符串
        )
        | prompt
        | llm
        | StrOutputParser()
    ...
    #     #     context=lambda x: retriever.invoke(x["input"]["question"]),
    #     #     question=lambda x: x["input"]["question"]
    #     # ) # 接收原始输入
    #     RunnableLambda(lambda x: x["question"]).assign(
