...
def test_direct_invocation():
    ...
    # RunnablePassthrough 版本  
    chain2 = RunnablePassthrough().assign(
        context=lambda x: retriever.invoke(x["question"]),
        question=lambda x: x["question"]
    )
    
    # 测试输入：这是 LangServe 实际传递的格式
    ...
    try:
        ...
        print("✅ RunnableLambda 成功")
    except Exception as e:
        print(f"❌ RunnableLambda 失败: {e}")
    
    try:
        result2 = chain2.invoke(test_input) 
        print("✅ RunnablePassthrough 成功")
    except Exception as e:
        print(f"❌ RunnablePassthrough 失败: {e}")
