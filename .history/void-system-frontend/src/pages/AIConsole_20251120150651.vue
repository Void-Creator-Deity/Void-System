...
@keyframes containerGlow {
  ...
  100% { box-shadow: 0 0 50px rgba(0, 204, 255, 0.25); }
}
...
.message:hover .message-text {
  ...
  border-color: var(--accent-primary);
  box-shadow: 0 8px 25px rgba(0, 204, 255, 0.15);
...
.input-wrapper :deep(.el-input__wrapper.is-focus) {
  border-color: var(--accent-primary);
  box-shadow: inset 0 0 10px rgba(0, 0, 0, 0.5), 0 0 20px var(--accent-primary);
}
...
.input-container :deep(.el-button)::before {
  content: '';
  position: absolute;
  top: 0;
  left: -100%;
...
.input-container :deep(.el-button:hover:not(:disabled))::before {
  left: 100%;
}
...
.console-footer::before {
  ...
  background: linear-gradient(90deg, transparent, var(--accent-primary), transparent);
  opacity: 0.6;
  animation: gradientMove 8s ease-in-out infinite alternate-reverse;
}
