...
// 模拟的学习步骤数据，实际应该从advisorResult中解析
const learningSteps = computed(() => {
  if (!advisorResult.value) {
    return []
  }
  // 实际项目中，应该从advisorResult中解析学习步骤
  return advisorResult.value.steps || [
    {
      title: '准备阶段',
      description: '收集相关资料和工具，为任务做好准备'
    },
    {
      title: '实施阶段',
      description: '按照计划逐步执行任务'
    },
    {
      title: '评估阶段',
      description: '检查任务完成情况，进行必要的调整'
    }
  ]
})

// 估计用时，实际应该从advisorResult中解析
const estimatedDuration = computed(() => {
  if (!advisorResult.value) {
    return '30 分钟'
  }
  return advisorResult.value.estimatedDuration || '30 分钟'
})
