...
- SQLite 与 Chroma 通过 Adapter 接入；HTTP 层只传递规范领域契约，不承载旧任务模型。
- 任务执行只提供 `/api/goals`、`/api/runs`、`/api/triggers` 和 Run/Step 命令接口；旧 `/api/tasks`、`/api/task-chains`、任务分类与其数据库表已完成数据迁移并删除。
...
- `DESIGN.md`：总体产品设计、规范架构、实施顺序和完成定义
- `PROJECT_RULES.md`：工程、迁移、注释、测试与交付强制规则
- `docs/MASTER_ENGINEERING_PROMPT.md`：可直接用于后续全项目整治的完整目标提示词
- `CONTEXT.md`：领域语言、架构不变量和迁移方向
- `docs/api-contract.md`：前端可依赖的现行接口合同
- `docs/adr/`：已确认的架构决策
- `docs/next-iteration-product-loop.md`：商城、系统伙伴、上下文工程、长期记忆和用户画像的产品输入
