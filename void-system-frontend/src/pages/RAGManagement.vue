...
import { Check, Edit, Refresh, Search, Upload, UploadFilled, Warning } from '@element-plus/icons-vue'
...
async function loadDocuments() { loading.value = true; try { const data = await knowledgeAdministrationApi.listDocuments(); documents.value = data?.documents || []; if ((currentPage.value - 1) * pageSize >= filteredDocuments.value.length) currentPage.value = 1 } catch (error) { ElMessage.error(getApiErrorMessage(error, '共享资料加载失败')) } finally { loading.value = false } }
