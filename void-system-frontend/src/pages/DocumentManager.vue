...
import { ChatDotRound, ChatLineRound, CircleCheck, Clock, Delete, Document as DocumentIcon, FolderOpened, Loading, MoreFilled, Picture as PictureIcon, RefreshLeft, RefreshRight, Search, Upload, View, WarningFilled } from '@element-plus/icons-vue'
...
async function loadDocuments() { loading.value = true; loadError.value = ''; try { const data = await documentApi.list({ retention: retentionView.value, status: retentionView.value === 'active' ? (filterStatus.value || undefined) : undefined, limit: pageSize.value, offset: (currentPage.value - 1) * pageSize.value }); documents.value = data?.documents || []; totalCount.value = data?.pagination?.total ?? documents.value.length } catch (error) { loadError.value = formatAxiosErrorMessage(error, '请检查服务连接后重试。已经上传的资料不会丢失。'); ElMessage.error('资料列表加载失败') } finally { loading.value = false } }
