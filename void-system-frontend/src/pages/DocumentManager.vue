  ...
  <div class="knowledge-page">
    <div class="knowledge-shell">
      <header class="page-intro">
        <div>
          <p class="eyebrow">资料馆</p>
          <h1>把自己的资料与共享馆藏放在一起</h1>
          <p class="page-copy">上传资料归你所有；共享馆藏可加入资料馆后直接用于知识问答，不会复制文件或索引。</p>
        </div>
      ...
      <section class="library-layout">
        <main class="library-panel">
          <header class="panel-heading">
            <div><h2>{{ libraryHeading }}</h2><p>{{ libraryDescription }}</p></div>
            <div class="library-toolbar">
              <el-segmented v-model="sourceView" :options="sourceOptions" size="small" aria-label="资料馆视图" @change="changeSourceView" />
              <el-segmented v-if="sourceView === 'uploads'" v-model="retentionView" :options="retentionOptions" size="small" aria-label="上传资料状态" @change="changeRetentionView" />
              <el-switch v-if="retentionView === 'active'" v-model="includeGlobalShared" inline-prompt active-text="全局" inactive-text="馆内" aria-label="扩展到全部共享馆藏" />
              <div class="library-controls" :class="{ 'is-archived': retentionView === 'archived' }">
                ...
                <el-select v-if="retentionView === 'active'" v-model="filterStatus" aria-label="资料状态" @change="resetAndLoadDocuments"><el-option label="全部状态" value="" /><el-option label="整理中" value="processing" /><el-option label="可以提问" value="completed" /><el-option label="整理失败" value="failed" /><el-option label="已取消" value="cancelled" /></el-select>
          ...
          <div v-else-if="searchFailure" class="search-status search-status--error" role="alert">
            <el-icon><WarningFilled /></el-icon>
            <div><strong>暂时无法在资料中查找</strong><p>{{ searchFailure }}</p></div>
            <el-button :loading="searching" @click="performKnowledgeSearch">重试</el-button>
          </div>

          <div v-if="loadError" class="library-error" role="alert">
            <el-icon><WarningFilled /></el-icon>
            <div><strong>资料列表暂时无法更新</strong><p>{{ loadError }}</p></div>
            <el-button :icon="RefreshRight" :loading="loading" @click="refreshLibrary">重新加载</el-button>
          </div>
          <div v-if="loading" class="state-panel"><el-icon class="is-loading"><Loading /></el-icon><span>正在加载资料…</span></div>
          <el-empty v-else-if="!documents.length && !loadError" :description="retentionView === 'archived' ? '回收站是空的' : '这里还没有资料，先上传一份开始吧。'" />
          <div v-else class="document-list">
            <article v-for="doc in documents" :key="doc.doc_id" class="document-row">
              ...
              <div class="document-body">
                <div class="document-title-row"><h3>{{ doc.title || '未命名资料' }}</h3><div class="document-title-tags"><el-tag size="small" effect="plain" :type="sourceTagType(doc)">{{ sourceLabel(doc) }}</el-tag><el-tag size="small" effect="plain" :type="retentionView === 'archived' ? 'info' : getStatusTagType(documentStatus(doc))">{{ retentionView === 'archived' ? '已归档' : getStatusText(documentStatus(doc)) }}</el-tag></div></div>
              ...
              <div class="document-actions">
                <template v-if="retentionView === 'archived'">
                  <el-tooltip content="恢复到资料馆" placement="top"><el-button type="primary" circle aria-label="恢复资料" @click="restoreDoc(doc)"><el-icon><RefreshLeft /></el-icon></el-button></el-tooltip>
                ...
                <template v-else-if="doc.can_manage">
                ...
                <template v-else>
                  <el-tooltip v-if="!doc.is_in_library" content="加入我的资料馆" placement="top"><el-button type="primary" plain circle aria-label="加入我的资料馆" :loading="libraryMutationId === doc.doc_id" @click="joinShared(doc)"><el-icon><CirclePlus /></el-icon></el-button></el-tooltip>
                  <el-tooltip v-else content="从我的资料馆移除" placement="top"><el-button circle aria-label="从我的资料馆移除" :loading="libraryMutationId === doc.doc_id" @click="leaveShared(doc)"><el-icon><Remove /></el-icon></el-button></el-tooltip>
                  <el-tooltip v-if="canAsk(doc)" content="围绕这份资料提问" placement="top"><el-button type="primary" circle aria-label="围绕资料提问" @click="askWithDocument(doc)"><el-icon><ChatLineRound /></el-icon></el-button></el-tooltip>
                  <el-tooltip content="查看摘要" placement="top"><el-button circle aria-label="查看资料摘要" @click="previewDocument(doc)"><el-icon><View /></el-icon></el-button></el-tooltip>
                </template>
              </div>
            </article>
          </div>
...
import { ChatDotRound, ChatLineRound, CircleCheck, CirclePlus, Clock, Delete, Document as DocumentIcon, FolderOpened, Loading, MoreFilled, Picture as PictureIcon, RefreshLeft, RefreshRight, Remove, Search, Upload, View, WarningFilled } from '@element-plus/icons-vue'
...
const sourceView = ref('library')
const includeGlobalShared = ref(false)
const libraryMutationId = ref('')
const sourceOptions = [{ label: '我的资料馆', value: 'library' }, { label: '我的上传', value: 'uploads' }, { label: '共享馆藏', value: 'shared' }]
...
const libraryHeading = computed(() => retentionView.value === 'archived' ? '回收站' : ({ library: '我的资料馆', uploads: '我的上传', shared: '共享馆藏' })[sourceView.value])
const libraryDescription = computed(() => {
  if (retentionView.value === 'archived') return '归档资料不会参与回答，可随时恢复或永久清除。'
  if (sourceView.value === 'shared') return '浏览全局共享馆藏；加入后会出现在你的资料馆，不会复制任何内容。'
  if (sourceView.value === 'uploads') return '这里的资料只属于你，可整理、编辑、归档与删除。'
  return totalCount.value + ' 份资料会参与默认知识问答。'
})
...
const canAsk = doc => !doc.is_archived && documentStatus(doc) === 'completed' && (doc.visibility === 'private' || doc.is_in_library)
const sourceLabel = doc => doc.visibility === 'private' ? '我的上传' : (doc.is_in_library ? '已加入' : '共享馆藏')
const sourceTagType = doc => doc.visibility === 'private' ? 'success' : (doc.is_in_library ? 'primary' : 'info')
function getStatusText(status) { return ({ queued: '等待整理', pending: '等待整理', processing: '正在整理', parsed: '正在整理', indexing: '正在整理', completed: '可以提问', failed: '整理失败', cancelled: '已取消' })[status] || '等待整理' }
function getStatusTagType(status) { return ({ queued: 'info', pending: 'info', processing: 'warning', parsed: 'warning', indexing: 'warning', completed: 'success', failed: 'danger', cancelled: 'info' })[status] || 'info' }
function statusDescription(status) { return ({ queued: '已收到资料，稍后会开始整理。', pending: '已收到资料，稍后会开始整理。', processing: '正在读取内容并整理为可提问的资料。', parsed: '正在整理内容，马上就可以提问。', indexing: '正在整理内容，马上就可以提问。', completed: '资料已整理完成，可以围绕它提问。', failed: '这份资料暂时无法整理。', cancelled: '已停止整理，可在后台任务中重新开始。' })[status] || '正在整理资料内容。' }
...
async function handleFileUpload(files) {
  ...
  currentFileName.value = files.length === 1 ? files[0].name : `正在上传 ${files.length} 份资料`
  uploadMessage.value = '正在上传…'
  try {
    const data = await documentApi.library.upload(formData, event => {
      uploadProgress.value = event.total ? Math.round((event.loaded * 100) / event.total) : 0
      uploadMessage.value = uploadProgress.value >= 100 ? '上传完成，正在加入处理…' : `正在上传 ${uploadProgress.value}%`
    })
    for (const result of Array.isArray(data?.results) ? data.results : []) {
      const job = result?.ingestion || (result?.job_id ? result : null)
      if (job?.job_id) backgroundProgress.upsertKnowledgeJob(job)
    }
    uploadMessage.value = '上传完成，资料会在后台自动处理。'
    uploadTitle.value = ''
    uploadTags.value = []
    ElMessage.success('资料已上传，正在后台处理')
    await Promise.all([loadDocuments(), loadStats()])
  } catch (error) {
    uploadMessage.value = '上传失败'
    ElMessage.error(formatAxiosErrorMessage(error, '上传失败，请稍后再试'))
  } finally {
    window.setTimeout(() => { uploading.value = false }, 1200)
  }
}

async function loadDocuments() { loading.value = true; loadError.value = ''; try { const data = await documentApi.library.list({ source: sourceView.value, retention: sourceView.value === 'uploads' ? retentionView.value : 'active', status: retentionView.value === 'active' ? (filterStatus.value || undefined) : undefined, limit: pageSize.value, offset: (currentPage.value - 1) * pageSize.value }); documents.value = data?.documents || []; totalCount.value = data?.pagination?.total ?? documents.value.length } catch (error) { loadError.value = formatAxiosErrorMessage(error, '请检查服务 connection and retry.'); ElMessage.error('资料列表加载失败') } finally { loading.value = false } }
async function loadStats() { try { stats.value = await documentApi.library.stats(sourceView.value) } catch (error) { console.warn('无法加载资料统计', error) } }
...
function changeSourceView() { if (sourceView.value !== 'uploads') retentionView.value = 'active'; currentPage.value = 1; filterStatus.value = ''; clearSearch(); refreshLibrary() }
...
async function performKnowledgeSearch() {
  ...
  searchResults.value = []
  try {
    const data = await documentApi.library.search(query, { includeGlobalShared: includeGlobalShared.value })
    searchResults.value = data?.results || []
    if (!searchResults.value.length) ElMessage.info('没有找到相关内容')
  } catch (error) {
    searchFailure.value = formatAxiosErrorMessage(error, '资料检索服务暂不可用，请稍后重试。')
  } finally {
    searching.value = false
  }
}
function clearSearch() { searchResults.value = []; searchFailure.value = ''; searchQuery.value = '' }
...
async function previewDocument(doc) { if (doc.visibility === 'official') { previewTitle.value = doc.title || '共享资料'; previewContent.value = doc.content_preview || '这份共享资料未提供可预览摘要。'; previewDialogVisible.value = true; return } try { const data = await documentApi.get(doc.doc_id, { includeArchived: retentionView.value === 'archived' }); previewTitle.value = doc.title || '资料内容'; previewContent.value = data?.content_preview || '暂无可预览内容'; previewDialogVisible.value = true } catch (error) { ElMessage.error(formatAxiosErrorMessage(error, '暂时无法查看这份资料')) } }
async function joinShared(doc) { libraryMutationId.value = doc.doc_id; try { await documentApi.library.joinShared(doc.doc_id); ElMessage.success('已加入我的资料馆'); await refreshLibrary() } catch (error) { ElMessage.error(formatAxiosErrorMessage(error, '暂时无法加入资料馆')) } finally { libraryMutationId.value = '' } }
async function leaveShared(doc) { libraryMutationId.value = doc.doc_id; try { await documentApi.library.leaveShared(doc.doc_id); ElMessage.success('已从我的资料馆移除'); await refreshLibrary() } catch (error) { ElMessage.error(formatAxiosErrorMessage(error, '暂时无法移除资料')) } finally { libraryMutationId.value = '' } }
...
async function rebuildKnowledge() {
  ...
  try {
    const data = await documentApi.rebuildIndex()
    for (const job of Array.isArray(data?.jobs) ? data.jobs : []) {
      if (job?.job_id) backgroundProgress.upsertKnowledgeJob(job)
    }
    ElMessage.success('资料已加入重新处理，完成后状态会自动更新。')
    await refreshLibrary()
  } catch (error) {
    ElMessage.error(formatAxiosErrorMessage(error, '暂时无法重新处理资料。'))
  } finally {
...
.search-results { margin: 18px 24px 0; padding: 16px; border: 1px solid color-mix(in srgb, var(--color-primary) 24%, var(--border-color)); background: color-mix(in srgb, var(--color-primary) 5%, var(--bg-secondary)); }.search-results-heading { display: flex; justify-content: space-between; gap: 12px; align-items: center; margin-bottom: 10px; }.search-results-heading div { display: grid; gap: 3px; }.search-results-heading span { color: var(--text-muted); font-size: 12px; }.search-result { display: grid; grid-template-columns: 24px 1fr; gap: 10px; width: 100%; padding: 10px 0; border: 0; border-top: 1px solid var(--border-color-light); background: transparent; color: var(--text-primary); cursor: pointer; text-align: left; }.search-result:first-of-type { border-top: 0; }.search-result:hover .result-content { color: var(--color-primary); }.search-status { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: 12px; margin: 18px 24px 0; padding: 14px 16px; border: 1px solid var(--border-color); background: var(--bg-secondary); }.search-status--error { border-color: color-mix(in srgb, var(--color-warning) 34%, var(--border-color)); background: color-mix(in srgb, var(--color-warning) 6%, var(--bg-secondary)); }.search-status > .el-icon { color: var(--color-warning); font-size: 20px; }.search-status strong { color: var(--text-primary); font-size: 13px; }.search-status p { margin: 3px 0 0; color: var(--text-secondary); font-size: 12px; line-height: 1.5; }.result-index { color: var(--color-accent); font-size: 12px; font-weight: 700; }.result-content { display: -webkit-box; overflow: hidden; -webkit-box-orient: vertical; -webkit-line-clamp: 2; line-height: 1.55; transition: color .2s ease; }
.library-error { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:12px; margin:16px 20px 0; padding:14px; border:1px solid color-mix(in srgb,var(--color-danger) 24%,var(--border-color)); border-radius:7px; background:color-mix(in srgb,var(--color-danger) 5%,var(--bg-secondary)); }.library-error > .el-icon { color:var(--color-danger); font-size:21px; }.library-error strong { color:var(--text-primary); font-size:13px; }.library-error p { margin:3px 0 0; color:var(--text-secondary); font-size:12px; line-height:1.5; }
