<template>
  <div class="admin-ai-page">
    ...
    <div v-if="loadError" class="page-error" role="alert">
      ...
      <div><strong>AI 服务设置暂时无法读取</strong><p>{{ loadError }}</p></div>
      <el-button :icon="Refresh" :loading="loading.read" @click="loadAiConfig">重新加载</el-button>
    </div>
...
import { Check, CircleCheck, Connection, Cpu, InfoFilled, Link, MagicStick, MoonNight, Refresh, WarningFilled } from '@element-plus/icons-vue'
...
const loadAiConfig = async () => {
  ...
  try {
    ...
    loadError.value = getApiErrorMessage(error, '请检查后端服务后重新加载。')
    ElMessage.error('AI 服务配置读取失败')
  } finally {
    loading.read = false
  }
}
