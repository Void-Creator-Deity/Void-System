...
- **Marketplace**: retired. A future entitlement or store capability requires a new ADR with fulfilled value, lifecycle, failure handling, and a complete user flow; it must not reuse the deleted currency model.
- **System companion**: define a permissioned Context Gateway through which the companion can read approved user data such as current conversation context, Goals and Runs, capabilities, and profile preferences.
...
- Memory and profile modules must remain optional so Workspace Core can migrate to another product shell.
