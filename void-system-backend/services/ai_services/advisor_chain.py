...
from services.ai_services.llm_factory import active_runtime_settings, get_chat_llm
...
""",
    ...
    try:
        ...
        msg = str(e).lower()
        runtime = active_runtime_settings() or RuntimeSettings.from_environment()
        should_retry_ollama = (
            ("not found" in msg or "404" in msg)
            and str(runtime.LLM_PROVIDER or "").lower() == "ollama"
        )
        if not should_retry_ollama:
            raise
        fallback_model = _pick_first_available_ollama_model(runtime.OLLAMA_BASE_URL)
...
def _pick_first_available_ollama_model(base_url: str) -> Optional[str]:
    try:
